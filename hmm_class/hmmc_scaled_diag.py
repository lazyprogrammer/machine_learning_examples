...
# Continuous-observation HMM with scaling and diagonal covariance.
import wave
import numpy as np
...
def mvn_pdf(x, mu, sigma):
    D = len(x)
    c = 1 / np.sqrt(((2*np.pi)**D) * sigma.prod())
    e = np.exp( -0.5*((x - mu) * (x - mu) / sigma).sum() )
    return c * e
...
class HMM:
    ...
    def fit(self, X, max_iter=30, eps=10e-2):
                ...
                for j in xrange(self.M):
                    for k in xrange(self.K):
                        for t in xrange(T):
                            ...
                            # update covariances
                            sigma_num_n[j,k] += gamma[t,j,k] * (x[t] - self.mu[j,k]) * (x[t] - self.mu[j,k])
