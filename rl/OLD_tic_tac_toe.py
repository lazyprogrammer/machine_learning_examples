...
if __name__ == '__main__':
  ...
  T = 10000
  cumulative_wins1 = np.zeros(T)
  cumulative_wins2 = np.zeros(T)
  cumulative_draws = np.zeros(T)

  for t in xrange(T):
    if t % 200 == 0:
      print t
    winner = play_game(p1, p2, Environment())
    cumulative_wins1[t] = 1*(winner == -1)
    cumulative_wins2[t] = 1*(winner == +1)
    cumulative_draws[t] = 1*(winner is None)

  # TODO: plot things to help us understand how well the agent has learned
  cumulative_wins1 = np.cumsum(cumulative_wins1) / (np.arange(T) + 1)
  cumulative_wins2 = np.cumsum(cumulative_wins2) / (np.arange(T) + 1)
  cumulative_draws = np.cumsum(cumulative_draws) / (np.arange(T) + 1)
  plt.plot(cumulative_wins1, label='p1 win rate')
  plt.plot(cumulative_wins2, label='p2 win rate')
  plt.plot(cumulative_draws, label='draw rate')
  plt.legend()
  plt.show()
